...
const server = http.createServer((request, response) => {
    ...
    if (method === 'GET' && url === '/participantes') {
        ...
        const contantoCidades = participantes.reduce((acumuladora, participante)=> {
            acumuladora[participante.cidade] = (acumuladora[participante.cidade] || 0) + 1
            return acumuladora
        }, {})
        console.log(contantoCidades)

        let quantidadeDeParticipantes = 0
        let cidadeComMaiornumeroDeParticipantes = 0
        Object.entries(contantoCidades).forEach(([cidade, count])=>{
            if(count > quantidadeDeParticipantes){
                quantidadeDeParticipantes = count
                cidadeComMaiornumeroDeParticipantes = cidade
            }
        })
        response.writeHead(200, {"Content-Type":"application/json"})
        response.end(
            JSON.stringify({
                ...
                "Cidade com maior quantidade de participantes": cidadeComMaiornumeroDeParticipantes
            })
        )
    } else {
        response.writeHead(404, { "content-Type": "application/json" })
